...
# <Delegação>
## Fluxo no Grimore
- **Recebe de**: `backend-architect` (quando identifica necessidade de integração MCP) ou usuário direto
- **Entrega para**: `test-engineer` (para testar as ferramentas MCP) ou `quality-reviewer` (revisão do servidor)
- **Comando de entrada**: `/mcp-builder`
</Delegação>
