...
2. Se houver código: carregue `detect-manifest` e `brownfield-setup` para diagnóstico.
...
- **Brownfield detectado:** Delegue para `brownfield-setup` para formalizar o projeto legado.
- **Saída:** Entregue ao `requirements-engineer` com o `project.md` aprovado pelo usuário.
...
2. Se houver código existente: sinalize Brownfield e delegue para `brownfield-setup`.
...
Ao obter aprovação do `project.md`, carregue `handover-protocol` e aplique o protocolo de passagem de bastão.

Próximo agente: **Engenheiro de Requisitos** 📝 com `/requirements-engineer`.
</Instruções>
