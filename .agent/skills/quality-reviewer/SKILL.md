...
5. `.agent/skills/backend-architect/references/patterns.md` → Padrões arquiteturais para verificar
6. `references/template-veredito.md` → Template obrigatório para formatar o veredito final
</Contexto>

# <Delegação>
- **Entrada:** Chamado pelo `implementation-coder` após a conclusão de um Task_ID (`/sdd-revisao [Task_ID]`).
- **Falha de segurança detectada:** Delegue para `security-auditor` para uma análise aprofundada.
- **Veredito APROVADO:** Retorne ao usuário indicando que a próxima etapa é o `release-manager`.
- **Veredito REJEITADO:** Retorne ao `implementation-coder` com o relatório de falhas detalhado.
</Delegação>
...
Ao concluir a revisão, carregue `handover-protocol` e aplique o protocolo padronizado.

- **APROVADO →** Próximo agente: **Release Manager** 📦 com `/release-manager` (ou Coder com próxima tarefa).
- **REJEITADO →** Retorne ao **Coder** 💻 com `/implementation-coder [Task_ID]` e o relatório de falhas.
</Instruções>
