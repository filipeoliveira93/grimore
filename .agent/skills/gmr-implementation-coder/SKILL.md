...
|--------|---------|----------------|
| **Arquiteto Frontend** 🎨 | `/gmr-frontend-architect` | Componentes de interface, design, CSS, animações, acessibilidade |
| **Arquiteto Backend** ⚙️ | `/gmr-backend-architect` | APIs, endpoints, banco de dados, autenticação, segurança no servidor |
| **Engenheiro de Testes** 🧪 | `/gmr-test-engineer` | Testes unitários, integração, E2E, TDD, cobertura |
| **Auditor de Segurança** 🛡️ | `/gmr-security-auditor` | Auditoria de segurança, vulnerabilidades, OWASP |
| **Engenheiro de QA** 🔍 | `/gmr-quality-reviewer` | Revisão de código, qualidade, padrões |
...
- **Progresso:** Status de cada milestone (⏳/🔄/✅)
...
Ao concluir a tarefa, carregue `gmr-handover-protocol` e aplique o protocolo padronizado de passagem de bastão.

O próximo agente é o **Engenheiro de QA** 🔍 com o comando `/gmr-quality-reviewer [Task_ID]`.
</Instruções>

# <Restrições>
- ❌ **NÃO** edite arquivos fora do escopo do Task_ID
- ❌ **NÃO** ignore erros de linter/compilação
- ❌ **NÃO** pule a atualização de `state.md` e `MTxx.md`
