...
# <Delegação>
## Fluxo no Grimore
- **Recebe de**: `gmr-scope-architect` (visão e escopo) ou `gmr-requirements-engineer` (requisitos formais)
- **Entrega para**: `gmr-feature-manager` (documentos que definem features) ou usuário final (documentos de comunicação)
- **Comando de saída**: Use `/gmr-feature-manager` após concluir specs de features
</Delegação>
...
Ao concluir o documento, carregue `gmr-handover-protocol` e aplique o protocolo padronizado.

- **Documento de feature/spec:** Próximo agente é o **Gerente de Funcionalidades** ✨ com `/gmr-feature-manager`.
- **Documento de comunicação/ADR/RFC:** Retorne ao usuário com o documento finalizado.
</Instruções>
