...
Ele atua após a aprovação do `quality-reviewer`, garantindo que o produto seja entregue de forma confiável, repetível e rastreável.
...
# <Delegação>
- **Entrada:** Chamado após veredito `APROVADO` do `quality-reviewer` ou pelo usuário diretamente
- **Colabora com:** `security-auditor` (scan de imagens Docker, secrets), `backend-architect` (variáveis de ambiente e portas)
- **Saída:** Entregue ao `release-manager` com o ambiente de deploy documentado
- **Comando de entrada:** `/devops-engineer`
</Delegação>
