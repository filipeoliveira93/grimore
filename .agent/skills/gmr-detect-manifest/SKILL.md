...
O Detector de Manifesto fornece evidências técnicas objetivas sobre a stack em uso, servindo de base para a entrevista de stack (`gmr-stack-interview`) e para o setup brownfield (`gmr-brownfield-setup`).
</Meta-Contexto>
...
# <Delegação>
- **Entrada:** Chamada por `gmr-scope-architect`, `gmr-brownfield-setup` ou `gmr-requirements-engineer`.
- **Saída:** Retorne o resumo de detecção ao agente chamador. Não crie arquivos — apenas reporte.
- **Se nenhum manifesto for encontrado:** Sinalize e delegue para `gmr-stack-interview`.
</Delegação>
