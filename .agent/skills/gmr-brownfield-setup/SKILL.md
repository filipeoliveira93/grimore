...
2. Use `gmr-detect-manifest` para identificar a stack automaticamente.
...
# <Delegação>
- **Entrada:** Chamada pelo `gmr-scope-architect` ao detectar um projeto Brownfield.
- **Stack não detectada:** Delegue para `gmr-stack-interview` para entrevistar o usuário sobre a stack existente.
- **Saída:** Entregue ao `gmr-requirements-engineer` com a estrutura `.grimore/` criada e pronta para documentação de requisitos.
</Delegação>

# <Instruções>
## FASE 1: DIAGNÓSTICO
1. Liste os arquivos do projeto e identifique estruturas legadas (`.grimore/spec.md`, `MT*.md` na raiz, etc.).
2. Execute `gmr-detect-manifest` para identificar a stack tecnológica existente.
...
Ao concluir a migração e a formalização da estrutura, carregue `gmr-handover-protocol` e aplique o protocolo padronizado.

Próximo agente: **Engenheiro de Requisitos** 📝 com `/gmr-requirements-engineer` para documentar os requisitos formais do projeto migrado.
</Instruções>
...
- ✅ **SEMPRE** use `gmr-detect-manifest` antes de perguntar ao usuário sobre a stack.
