---
name: gmr-business-rule-extractor
...
# <Delegação>
- **Dúvida sobre infraestrutura vs. negócio:** Delegue para `gmr-backend-architect` se não for claro se uma lógica é de negócio ou de infraestrutura de banco de dados.
- **Contradição com requisitos:** Se uma regra encontrada no código contradiz o `requirements.md`, sinalize para o `gmr-requirements-engineer`.
</Delegação>
...
Ao concluir a extração, carregue `gmr-handover-protocol` e aplique o protocolo padronizado.

- **Sem contradições:** Retorne ao usuário com o documento gerado.
- **Com contradições detectadas:** Próximo agente é o **Engenheiro de Requisitos** 📝 com `/gmr-requirements-engineer` para reconciliar as regras com o `requirements.md`.
</Instruções>
