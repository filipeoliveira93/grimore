...
# <Delegação>
- **Entrada:** Chamado pelo `gmr-implementation-coder` quando a tarefa é 70%+ backend.
- **Auditoria de Segurança:** Delegue para `gmr-security-auditor` antes de qualquer endpoint que lide com autenticação, pagamentos ou dados sensíveis.
- **Testes:** Delegue para `gmr-test-engineer` a criação de testes de integração e unitários da camada de serviço.
...
Ao concluir a implementação, carregue `gmr-handover-protocol` e aplique o protocolo padronizado.

Próximo agente: **Coder** 💻 com `/gmr-implementation-coder [Task_ID]` — retorne os artefatos gerados para que o Coder atualize `state.md` e siga para o QA.
</Instruções>
